...
class ValueIterationAgent(ValueEstimationAgent):
    ...
    def runValueIteration(self):
        ...
        for i in range(self.iterations):
            # temporary counter to keep track of values in state-action iteration
            tempCounter = util.Counter()
            for state in self.mdp.getStates():
                maxValue = float("-inf")
                for action in self.mdp.getPossibleActions(state):
                    qValue = self.computeQValueFromValues(state, action)
                    # find max q value and set to temporary counter
                    if qValue > maxValue:
                        maxValue = qValue
                    tempCounter[state] = maxValue
            # update values based on the temp counter
    ...
    def computeQValueFromValues(self, state, action):
        ...
        """
        "*** YOUR CODE HERE ***"  
        # find pairs of the next state and the probability from MDP
        actionProbPairs = self.mdp.getTransitionStatesAndProbs(state, action)
        total = 0
        for nextState, prob in actionProbPairs:
    ...
    def computeActionFromValues(self, state):
        ...
        """
        "*** YOUR CODE HERE ***"
        # set bestAction equal to None as default; if does not run in line 118, returns None
        bestAction = None
        maxValue = float("-inf")
        for action in self.mdp.getPossibleActions(state):
            qValue = self.computeQValueFromValues(state, action)
...
class AsynchronousValueIterationAgent(ValueIterationAgent):
    ...
    def runValueIteration(self):
        "*** YOUR CODE HERE ***"
        # keep track of number of states
        states = self.mdp.getStates()
        for i in range(self.iterations):
            # len(states) can (and likely) is < num iterations, so 
            # use modular to keep going through the value iteration
            ...
            if not self.mdp.isTerminal(state):
                ...
                # update value based on best action taken
...
class PrioritizedSweepingValueIterationAgent(AsynchronousValueIterationAgent):
    ...
    def runValueIteration(self):
        ...
        for i in range(self.iterations):
            ...
            for p in predecessors[tempState]:
                if not self.mdp.isTerminal(tempState):
                    values = []
